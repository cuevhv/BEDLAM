...
def get_data(csv_data, cam_csv_base, gt_smplx_folder, image_folder_base, output_folder, fps, scene_name):
    ...
    for idx, comment in tqdm(enumerate(csv_data['Comment']), total=len(csv_data['Comment'])):
        if 'sequence_name' in comment:
            ...
            body_data["smplx_param_orig"] = np.load(os.path.join(gt_smplx_folder, body_data["person_id"], sequence_id, 'motion_seq.npz'))
            body_data["gender_sub"] = body_data["smplx_param_orig"]['gender_sub'].item()
...
def process_scenes(scene_data, smplx_models, scale_factor, downsample_mat):
    ...
    for img_idx, image_path in (enumerate(all_images)):
        ...
        for body_data in scene_data["bodies"]:
            ...
            if smplx_param_ind > body_data["smplx_param_orig"]['poses'].shape[0]:
                break
            pose = body_data["smplx_param_orig"]['poses'][smplx_param_ind]
            transl = body_data["smplx_param_orig"]['trans'][smplx_param_ind]
            beta = body_data["smplx_param_orig"]['betas']
            motion_info = body_data["smplx_param_orig"]['motion_info']

            gender = body_data["smplx_param_orig"]['gender']
